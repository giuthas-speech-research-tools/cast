...
def extractGrids(table, mainGrid):
    ...
    transcriptions = loadTransRules("/Users/jpalo/tyot/vaikkari/source/python/ex4_phone_transcription_rules.csv")
...
def correctTranscription(grid):
    segmentTier = grid.pop()
    wordTier = grid.pop()

    word = wordTier.intervals[1].mark.lower()
    for interval in wordTier:
        if interval.mark == 'sp' or interval.mark == '':
            continue
        else:
    ...
    seg_iter = segmentTier.__iter__()
    i = 0
    for interval in seg_iter:
        if ('sp' in interval.mark 
            or 'beep' in interval.mark 
            or interval.mark == ''):
            continue
        else:
            # handles oe and ae as special cases
            if word[i] in 'ao' and len(word) > i+1:
                interval.mark = word[i:i+2] 
            else:
                interval.mark = word[i]
                i += 1
...
def main(args):
    ...
    #grids = map(correctTranscription, grids)
